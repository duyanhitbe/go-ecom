// Code generated by MockGen. DO NOT EDIT.
// Source: github.com/duyanhitbe/go-ecom/internal/repositories (interfaces: Querier)
//
// Generated by this command:
//
//	mockgen -destination=test/mocks/mock_querier.go -package=mock github.com/duyanhitbe/go-ecom/internal/repositories Querier
//

// Package mock is a generated GoMock package.
package mock

import (
	context "context"
	reflect "reflect"

	repositories "github.com/duyanhitbe/go-ecom/internal/repositories"
	gomock "go.uber.org/mock/gomock"
)

// MockQuerier is a mock of Querier interface.
type MockQuerier struct {
	ctrl     *gomock.Controller
	recorder *MockQuerierMockRecorder
	isgomock struct{}
}

// MockQuerierMockRecorder is the mock recorder for MockQuerier.
type MockQuerierMockRecorder struct {
	mock *MockQuerier
}

// NewMockQuerier creates a new mock instance.
func NewMockQuerier(ctrl *gomock.Controller) *MockQuerier {
	mock := &MockQuerier{ctrl: ctrl}
	mock.recorder = &MockQuerierMockRecorder{mock}
	return mock
}

// EXPECT returns an object that allows the caller to indicate expected use.
func (m *MockQuerier) EXPECT() *MockQuerierMockRecorder {
	return m.recorder
}

// CountUser mocks base method.
func (m *MockQuerier) CountUser(ctx context.Context) (int32, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "CountUser", ctx)
	ret0, _ := ret[0].(int32)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}

// CountUser indicates an expected call of CountUser.
func (mr *MockQuerierMockRecorder) CountUser(ctx any) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "CountUser", reflect.TypeOf((*MockQuerier)(nil).CountUser), ctx)
}

// CreateUser mocks base method.
func (m *MockQuerier) CreateUser(ctx context.Context, arg *repositories.CreateUserParams) (*repositories.User, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "CreateUser", ctx, arg)
	ret0, _ := ret[0].(*repositories.User)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}

// CreateUser indicates an expected call of CreateUser.
func (mr *MockQuerierMockRecorder) CreateUser(ctx, arg any) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "CreateUser", reflect.TypeOf((*MockQuerier)(nil).CreateUser), ctx, arg)
}

// FindOneUserByUsername mocks base method.
func (m *MockQuerier) FindOneUserByUsername(ctx context.Context, username string) (*repositories.User, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "FindOneUserByUsername", ctx, username)
	ret0, _ := ret[0].(*repositories.User)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}

// FindOneUserByUsername indicates an expected call of FindOneUserByUsername.
func (mr *MockQuerierMockRecorder) FindOneUserByUsername(ctx, username any) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "FindOneUserByUsername", reflect.TypeOf((*MockQuerier)(nil).FindOneUserByUsername), ctx, username)
}

// FindUser mocks base method.
func (m *MockQuerier) FindUser(ctx context.Context, arg *repositories.FindUserParams) ([]*repositories.User, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "FindUser", ctx, arg)
	ret0, _ := ret[0].([]*repositories.User)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}

// FindUser indicates an expected call of FindUser.
func (mr *MockQuerierMockRecorder) FindUser(ctx, arg any) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "FindUser", reflect.TypeOf((*MockQuerier)(nil).FindUser), ctx, arg)
}
